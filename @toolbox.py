...
init_month = 1
data_x_mz = pd.read_excel("todo_ok.xlsx", sheet_name='Datos por MZ',
...
def relabel_data(pd_data: pd.DataFrame) -> pd.DataFrame:
    ...
    data_to_cluster = full_data_cluster[
            (full_data_cluster['status'] == 0)].loc[:,
    ...
    pos.insert(pos.shape[1], 'status_clf',
               np.ones(pos.shape[0]).reshape((-1, 1)), True)
    ...
    full_data.columns = ['MZ', 'date', 'week', '1_week_ago', '2_week_ago',
                         '3_week_ago', '4_week_ago', '5_week_ago',
                         '6_week_ago', '7_week_ago', '8_week_ago', 'status']
    return full_data
...
def train_model(model_name: str, full_data:pd.DataFrame,
                k_data: str, train_test=False) -> StackingClassifier:
    """_summary_
    ...
    print_sms(f"starting training of model '{model_name}' at ",
              Timestamp.now())
    ...
    training_data = full_data[(full_data['date'] > first_date) &
                              (full_data['date'] < predict_date_begin)]
    ...
    print_sms(f'Percentage of testing set\
    ...
    if train_test:
        ...
        print(f'{model_name}__precision:{round(prec, 4)}\
...
def select_model_to_run(model_name:str, k_data:str):
    ...
    if k_data == 'relabeled':
        rf_estimator, bag_estimator, pas_estimator, ada_estimator, bg_pg_ada = est_table.best_data_labeled
        rf_max_leaf, bag_max_leaf, pas_max_leaf, ada_max_leaf, bg_pg_ada= est_table.max_leaf_data_labeled
    else:
        rf_estimator, bag_estimator, pas_estimator, ada_estimator, bg_pg_ada = est_table.best_data_original
        rf_max_leaf, bag_max_leaf, pas_max_leaf, ada_max_leaf, bg_pg_ada = est_table.max_leaf_data_original
    raf_clf = RandomForestClassifier(n_estimators=int(rf_estimator), random_state=42, max_features=1.0, n_jobs=-1)
    ...
    if model_name == 'bag_clf-past_clf-adaboost':
        ...
        bag_clf = BaggingClassifier(RandomForestClassifier(), n_estimators=int(bag_estimator), bootstrap=True,
                                    n_jobs=-1)
        pas_clf = BaggingClassifier(RandomForestClassifier(), n_estimators=int(pas_estimator), bootstrap=False,
                                    n_jobs=-1)
        voting_clf = StackingClassifier(estimators=[('bag', bag_clf), ('pas', pas_clf), ('ada', ada_clf)],
        ...
        raf_clf = RandomForestClassifier(max_leaf_nodes=int(ada_max_leaf), n_estimators=int(rf_estimator),
                                         random_state=42, n_jobs=-1)
        voting_clf = AdaBoostClassifier(raf_clf, n_estimators=int(ada_estimator), algorithm="SAMME.R",
                                        learning_rate=0.05)
    elif model_name == 'pas_clf':
        print('running pas_clf model')
        raf_clf = RandomForestClassifier(max_leaf_nodes=int(pas_max_leaf), n_estimators=int(rf_estimator),
                                         random_state=42, n_jobs=-1)
        voting_clf = BaggingClassifier(RandomForestClassifier(), n_estimators=int(pas_estimator), bootstrap=False,
                                       n_jobs=-1)
    else: # bag_clf
        print('running bag_clf model')
        raf_clf = RandomForestClassifier(max_leaf_nodes=int(bag_max_leaf), n_estimators=int(rf_estimator),
                                         random_state=42, n_jobs=-1)
        voting_clf = BaggingClassifier(RandomForestClassifier(), n_estimators=int(bag_estimator), bootstrap=True,
                                       n_jobs=-1)
    return voting_clf
...
def test_model(voting_clf: StackingClassifier, full_data: pd.DataFrame) -> tuple:
    ...
    for i in range(init_month, 13):
        ...
        X_predict_data = full_data[(full_data['date'] >= tmp_predict_date_begin) &
    ...
    all_results = pd.DataFrame({'False_neg': false_negative,
...
def get_model_performance(all_prediction_2022: dict, full_data: pd.DataFrame, all_results: pd.DataFrame) -> tuple:
    ...
    for i in range(init_month,13):
        ...
        negative_zone_in_period = full_data[(full_data.date >= date_begin) & (full_data.date < date_end) & (full_data.status == 0)]
        prec_score = 0
        rec_score = 0
        f1 = 0
        if positive_zone_in_period.shape[0] > 1:
            # total positive zones predicted by model
            prediction_of_the_month = Counter(all_prediction_2022.get(months[i-1])).keys()
            total_pos_model_month.append(len(prediction_of_the_month))
            zones_matches = set(prediction_of_the_month).intersection(set(positive_zone_in_period.MZ))  # True positive
            zones_matches_FP = set(negative_zone_in_period.MZ).intersection(set(prediction_of_the_month))
    ...
    all_res = pd.DataFrame({'Zones_pred_model': total_pos_model_month,
                            'zones_detected': real_month, 'Zones_pred_model_match': all_results['True_pos'],
...
def run_tool_box() -> tuple:
    ...
    print(f'A total of {len(correct_models)} ({correct_models}) models will be executed')
...
def save_model(model, file_path):
    ...
    _saved_model["X_train"] = model.X_train.tolist() if model.X_train is not None else "None",
    _saved_model["y_train"] = model.y_train.tolist() if model.y_train is not None else "None"
...
def find_best_estimator(models_to_run: tuple, ask_range: False, file_number: 0) -> None:
    ...
    for md in models_to_run:
        ...
        print(len(X_data))
        print(f"Running model '{model}' with '{data_to_use}' dataset to find best estimator")
        results_find.append(f"Running model '{model}' with '{data_to_use}' dataset to find best estimator")
        # dataset = process_data(data_to_use)
        # if '-' in model:
            # print('breaking for...')
            # break
        while selected_range == '':
            ...
            print_estimators_table()
            print("Please enter a range (e.g. 1-20) or leave it in blank to take range of model")
            print(f"'{model}' from table. The range will be considered for all models of your")
            if ask_range:
        ...
        grid_SearCV = GridSearchCV(selected_model, param_grid=param_grid, n_jobs=-1, verbose=1, cv=5)
        ...
        if not find_grid:
            ...
            if not pre_train_test:
                ...
                for i in range(1, 2):
                    ...
                    print(f'Training and testing model in training datasets. Run number {i}')
                    model_trained = train_model(model, full_data=dataset, k_data=data_to_use, train_test=test_or_not)
                    joblib.dump(model_trained, f'./models_saved/train_test_model_{model}_run_{i}.pkl')
                    print(f'The model {model} has been saved, filename: (train_test_model_{model}_run_{i}.pkl)')
        else:
